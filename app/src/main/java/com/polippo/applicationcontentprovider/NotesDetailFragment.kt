    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        if(arguments != null && arguments?.getLong(EXTRA_ID) != 0L){
            ...
            if(cursor?.moveToNext() as Boolean){
                newNote = false
                noteEditTitle.setText(cursor.getString(cursor.getColumnIndex(NotesDataBaseHelper.TITLE_NOTES)))
                noteEditDescription.setText(cursor.getString(cursor.getColumnIndex(NotesDataBaseHelper.DESCRIPTION_NOTES)))
            }
            cursor.close()
        }
